...
function validarFormulario(acao) {
  ...
  if (!carro || !anoInput?.value.trim() || !placaInput?.value.trim()) {
  ...
  // Validação dos checkboxes (exatamente 15 marcados)
  const checkboxesMarcados = document.querySelectorAll('input[name="ficha"]:checked');
  if (acao === 'imprimir' && checkboxesMarcados.length !== 15) {
    alert('Para imprimir, é necessário marcar exatamente 15 itens.');
    return;
  }
